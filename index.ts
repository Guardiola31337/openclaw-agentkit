...
import { createAgentkitSessionEndHook } from "./src/external-verification-grants.js";
...
const agentkitPlugin: OpenClawPluginDefinition = definePluginEntry({
  ...
  register(api) {
    ...
    const externalVerification = createAgentkitExternalVerificationRuntime(api);
    ...
    api.on("session_end", createAgentkitSessionEndHook(api));
    api.registerCli(
